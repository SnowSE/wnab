...
public record TransactionSplitRecord(int? CategoryAllocationId, int TransactionId, decimal Amount, string? Notes);
...
public record EditTransactionSplitRequest(int Id, int? CategoryAllocationId, decimal Amount, string? Description);
